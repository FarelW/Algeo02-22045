...
func main ()  {
	...
	app.Post("/api/upload", func(c *fiber.Ctx) error {
		...
		if isColor {
			...
			// log.Println("vector:",textureVector)
	...
	// app.Post("/api/scraping", func(c *fiber.Ctx) error {
	// 	proccesstype := c.FormValue("proccesstype")
	// 	if proccesstype == "" {
	// 		log.Printf("Error detecting process type: not provided\n")
	// 		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Process type not found"})
	...
	app.Post("/api/webcam/snapshot", func(c *fiber.Ctx) error {
		...
		if err := c.BodyParser(&imageData); err != nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "request body parsing error"})
		}
	
		// Decode the base64 image
		// Process or store the image as needed
		// ...
	
		return c.Status(fiber.StatusOK).JSON(fiber.Map{"message": "Image received"})
	})
